    ...
    def main(self):
        ...
        with tf.Session() as self.sess:
            ...
            while True:
                ...
                self.image=cv2.resize(self.image,(frame_width//2,frame_height//2))
                ...
                if len(parking_area) > 0:
                    ...
                    for pa in parking_area:
                        ...
                        if parking_name not in self.print_status.keys():
                            ...
                            park_data = {"id": p, "color": "green", "text": ""}
                        ...
                        if len(car_dist_to_park_dist) > 0:
                            if min(car_dist_to_park_dist) < pa[1] and not self.park_status[parking_name]:
                                ...
                                if park_status:
                                    if self.park_in_count[parking_name] == 0:
                                        self.park_in_count[parking_name] = datetime.datetime.now()
                                    park_status_time_ = (datetime.datetime.now() - self.park_in_count[parking_name])
                                    park_status_time = park_status_time_.total_seconds()

                                    if park_status_time >= 30: #seconds
                                        self.park_in_count[parking_name] = 0
                                        ...
                                        if number_plate != None:
                                            park_data = {"id": p, "color": "red", "text": "occupied by " + str(number_plate)}
                                            data[p-1] = park_data
                                        if number_plate == None:
                                            park_data = {"id": p, "color": "red", "text": "occupied by plate not recognized\n"}
                                            data[p-1] = park_data
                            ...
                            if self.park_status[parking_name]:
                                ...
                                if not park_status_:
                                    if self.park_out_count[parking_name] == 0:
                                        self.park_out_count[parking_name] = datetime.datetime.now()
                                    park_out_status_time_ = (datetime.datetime.now() - self.park_out_count[parking_name])
                                    park_out_status_time = park_out_status_time_.total_seconds()

                                    if park_out_status_time >= 30: #seconds
                                        self.park_out_count[parking_name] = 0
                                        ...
                                        park_data = {"id": p, "color": "green", "text": " Parking available"}
